...
fn select_58() {
    ...
    let select = SelectStatement::new()
        ...
        .to_owned();
    assert_eq!(
        select.with(with_clause).to_string(BigQueryQueryBuilder),
        [
            r#"WITH `cte` AS"#,
...
fn select_59() {
    ...
    assert_eq!(
        query.to_string(BigQueryQueryBuilder),
...
fn select_61() {
    assert_eq!(
        Query::select()
            ...
            .build(BigQueryQueryBuilder),
        (
            r#"SELECT `character` FROM `character` WHERE `character` LIKE $1 ESCAPE '\\'"#
                .to_owned(),
            Values(vec!["A".into()])
        )
    );
...
fn insert_6() -> error::Result<()> {
    ...
    insert
        ...
        .columns([Glyph::Id, Glyph::Image, Glyph::Aspect])
        .select_from(select)?;
    let sql = insert.with(with_clause).to_string(BigQueryQueryBuilder);
    assert_eq!(
        sql.as_str(),
        [
...
fn escape_1() {
    let test = r#" "abc" "#;
    assert_eq!(
        BigQueryQueryBuilder.escape_string(test),
        r#" \"abc\" "#.to_owned()
    );
    assert_eq!(
        BigQueryQueryBuilder.unescape_string(BigQueryQueryBuilder.escape_string(test).as_str()),
        test
    )
}
...
fn union_1() {
    assert_eq!(
        Query::select()
            ...
            .to_string(BigQueryQueryBuilder),
        [
            r#"SELECT `character` FROM `character` UNION DISTINCT SELECT `character` FROM `character`"#,
...
fn sub_query_with_fn() {
    ...
    assert_eq!(
        select.to_string(BigQueryQueryBuilder),
