...
use crate::extension::mysql::{
    ExplainTable, ExplainTableTarget, MySqlExplainOptions, MySqlExplainSchemaSpec,
};
...
pub struct ExplainStatement {
    pub(crate) statement: Option<Box<ExplainableStatement>>,
    #[cfg(feature = "backend-postgres")]
...
impl ExplainStatement {
    ...
    pub fn statement(mut self, statement: impl Into<ExplainableStatement>) -> Self {
        self.statement = Some(Box::new(statement.into()));
        self
    }
}
...
impl ExplainStatement {
    ...
    fn set_analyze(&mut self, analyze: bool) {
        #[cfg(feature = "backend-postgres")]
        {
            self.pg_opts.analyze = Some(analyze);
        }
        #[cfg(feature = "backend-mysql")]
        {
            self.mysql_opts.analyze = Some(analyze);
        }
    }

    fn set_format(&mut self, format: ExplainFormat) {
        #[cfg(feature = "backend-postgres")]
        {
            self.pg_opts.format = Some(format);
        }
        #[cfg(feature = "backend-mysql")]
        {
            self.mysql_opts.format = Some(format);
        }
    }
}
...
impl ExplainStatement {
    ...
    pub fn table(table: impl Into<ExplainTable>) -> Self {
        let mut statement = Self::new();
        statement.set_table(table.into());
        statement
    }
    ...
    fn set_table(&mut self, table: ExplainTable) {
        self.mysql_opts.table = Some(table.table);
        self.mysql_opts.target = table.target;
    }
}
