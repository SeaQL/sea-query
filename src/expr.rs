...
//!
//! [`Expr`] is the expression common among select fields, where clauses and many other places.

use crate::{func::*, query::*, types::*, value::*};

/// Represents a Simple Expression in SQL.
///
/// [`Expr`] is a node in the expression tree and can represent identifiers, function calls,
...
impl Expr {
    ...
    pub fn asterisk() -> Self {
        Self::Column(Asterisk.into_column_ref())
    }
    ...
    /// );
    /// ```
    pub fn col(n: impl IntoColumnRef) -> Self {
    ...
    ///     .from(Char::Table)
    ///     .and_where(
    ///         Expr::tuple([Expr::col(Char::SizeW).into(), Expr::val(100)])
    ///             .lt(Expr::tuple([Expr::val(500), Expr::val(100)])))
    ///     .to_owned();
    ///
    /// assert_eq!(
    ///     query.to_string(MysqlQueryBuilder),
    ...
    /// );
    /// ```
    pub fn tuple(n: impl IntoIterator<Item = Expr>) -> Self {
        Expr::Tuple(n.into_iter().collect::<Vec<Expr>>())
    }

    #[deprecated(since = "0.29.0", note = "Please use the [`Asterisk`]")]
    pub fn table_asterisk(t: impl IntoIden) -> Self {
    ...
    /// );
    /// ```
    pub fn cust(s: impl Into<String>) -> Self {
        Expr::Custom(s.into())
    }
    ...
    where
        ...
        E: Into<Expr>,
    {
        Expr::CustomWithExpr(s.into(), vec![expr.into()])
    }
    ...
    {
        Expr::CustomWithExpr(s.into(), v.into_iter().collect())
    }
